...
async def test_pending_duplicate_retries_when_owner_fails(raises: bool) -> None:
    entered = asyncio.Event()
    release = asyncio.Event()
    calls = 0

    async def fail_then_success() -> bool | None:
        nonlocal calls
        calls += 1
        if calls == 1:
            entered.set()
    ...
    payload = _triggering_payload(COMMENT_ID)
    owner = asyncio.create_task(guard.dispatch_comment_once(payload, fail_then_success))
    await entered.wait()
    duplicate = asyncio.create_task(guard.dispatch_comment_once(payload, fail_then_success))
    ...
    assert isinstance(owner_result[0], RuntimeError) is raises
    assert calls == 2
    assert guard._comment_delivery_states[COMMENT_ID] == "succeeded"
...
async def test_route_concurrent_duplicate_claims_once_and_commits_success(
    ...
    await entered.wait()
    assert calls == 1
    assert isinstance(guard._comment_delivery_states[COMMENT_ID], asyncio.Future)
    release.set()
...
async def test_trace_comment_failure_does_not_reopen_delivery(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    async def none(*_args, **_kwargs) -> None:
        return None

    async def issue_details(*_args, **_kwargs) -> dict:
        return issue

    async def dispatch(*_args, **_kwargs) -> dict[str, str]:
        nonlocal dispatches
        dispatches += 1
        return {"run_id": "run-1"}

    async def failing_trace(*_args, **_kwargs) -> None:
    ...
    monkeypatch.setattr(service.common, "react_to_linear_comment", none)
    monkeypatch.setattr(service.common, "generate_thread_id_from_issue", lambda _id: "thread-1")
    monkeypatch.setattr(service.common, "fetch_linear_issue_details", issue_details)
    monkeypatch.setattr(service.common, "upsert_agent_thread_owner_metadata", none)
    monkeypatch.setattr(service.common, "dispatch_agent_run", dispatch)
    monkeypatch.setattr(service.common, "post_linear_trace_comment", failing_trace)

    payload = _triggering_payload(COMMENT_ID)
    args = (issue, {"owner": "speedbay", "name": "warehouse"})
    await guard.dispatch_comment_once(payload, service.process_linear_issue, *args)
    await guard.dispatch_comment_once(copy.deepcopy(payload), service.process_linear_issue, *args)

    assert dispatches == 1
    assert trace_attempts == 1
...
async def test_dispatch_comment_once_identity_and_capacity_boundaries() -> None:
    ...
    assert guard._comment_delivery_states["pending"] is pending
    assert "succeeded-0" not in guard._comment_delivery_states

    guard._comment_delivery_states.clear()
    for i in range(guard._SEEN_COMMENTS_MAX):
        guard._comment_delivery_states[f"pending-{i}"] = asyncio.get_running_loop().create_future()

    entered = asyncio.Event()
    release = asyncio.Event()

    async def blocked_dispatch(label: str) -> None:
        calls.append(label)
        entered.set()
        await release.wait()
