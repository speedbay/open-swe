...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("Generated by Claude Code", "ai-attribution"),
        ("Made by [Open SWE]", "ai-attribution"),
        ("Update OpenAI embedding config", None),
        ("Generated with deterministic seed", None),
        ("Open-Sweeney", None),
    ],
)
def test_attribution_parity(text: str, rule: str | None) -> None:
    verdict = check_attribution(text)
    assert verdict is None if rule is None else _rule(verdict) == rule


@pytest.mark.parametrize(
    ("text", "rule"),
    [
        ("ran git add -A", "explicit-staging"),
        ("git add agent/a.py tests/a.py", None),
    ],
)
def test_observable_staging_parity(text: str, rule: str | None) -> None:
    verdict = check_observable_staging(text)
    assert verdict is None if rule is None else _rule(verdict) == rule
...
@pytest.mark.parametrize(
    ("line", "rule"),
    [
        ("Refs: OPE-1", "non-closing-ref"),
        ("Part of OPE-122", "non-closing-ref"),
        ("Part of: OPE-122", "non-closing-ref"),
        ("Related to OPE-122", "non-closing-ref"),
        ("Related to: OPE-122", "non-closing-ref"),
        ("The parser is part of the gate.", None),
    ],
)
def test_body_non_closing_metadata_parity(line: str, rule: str | None) -> None:
    verdict = check_body_sections(_body() + f"\n{line}\n", ISSUE)
    assert verdict is None if rule is None else _rule(verdict) == rule
...
def test_commit_message_contract_and_no_subject_exemptions() -> None:
    ...
    for subject in ("Merge branch 'main' into feature", f'Revert "{ISSUE}: port rules"'):
        assert _rule(check_commit_message(subject, ISSUE)[0]) == "title-format"


def test_subject_independent_checks_do_not_need_an_issue() -> None:
    assert {v.rule for v in check_subject_independent("Made by [Open SWE]\ngit add -A")} == {
        "ai-attribution",
        "explicit-staging",
    }
