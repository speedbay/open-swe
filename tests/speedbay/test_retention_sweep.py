...
class _FakeClient:
    def __init__(
        self, threads: list[dict[str, Any]], crons: list[dict[str, Any]] | None = None
    ) -> None:
        self.threads = _FakeThreads(threads)
        self.crons = _FakeCrons(crons or [])
...
async def test_sweep_pages_threads_and_crons(monkeypatch) -> None:
    ...
    assert [call["offset"] for call in client.crons.search_calls] == [0, 2]
