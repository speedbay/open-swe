# Operations — running the Speed Bay Open SWE deployment

How to run and operate the deployment. Fork conventions and the
upstream-merge re-check contract live in [`FORK.md`](FORK.md); the ordered
zero-to-running onboarding path lives in [`SETUP.md`](SETUP.md).
...
```python
# .venv/bin/python -m asyncio — one record per dev
...
rule: a run that dies immediately at its first model call with a quota/billing
4xx from `api.fireworks.ai` is a **billing/cap event, not a code fault** — check
the Fireworks balance before debugging anything. A 401 `UNAUTHORIZED` indicates
an auth failure, not a billing event.
