...
| 3 | `docker` branch calling `validate_startup_config()` (OPE-7), which also schedules the boot-time verify-sweep (OPE-53) and subscription-health (OPE-68) cron ensures | inside `validate_sandbox_startup_config()` in `agent/utils/sandbox.py`; cron-ensure logic lives in `agent/speedbay/verify_sweep_cron.py` and `agent/speedbay/subscription_health_cron.py` |
...
| 6 | `task == "verify_sweep"` (OPE-42) and `task == "subscription_health"` (OPE-68) scheduler branches | inside `_launch()` in `agent/scheduler.py`, after the `reconcile` branch; logic lives in `agent/speedbay/verify_sweep.py` and `agent/speedbay/subscription_health.py`. Their crons are ensured idempotently at boot (registration #3); `speedbay/ensure_verify_sweep_cron.py` is list/inspect only |
