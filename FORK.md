...
| `tests/e2e/playwright.config.ts`, `tests/e2e/fake_llm.py`, `tests/e2e/tests/dashboard.spec.ts`, `tests/e2e/tests/plan_review.spec.ts`, `tests/e2e/tests/full_flow.spec.ts`, `tests/e2e/README.md` | OPE-113 optimizes retry artifacts and busy timing, scopes plan assertions to their Slack thread, replaces a fixed negative wait with the webhook verdict, and documents fork compatibility CI. | Direct edits are cheaper than fork-only wrappers because timing and thread assertions live at these existing test seams; no production agent logic changes. |
