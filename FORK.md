...
| `agent/speedbay/` | **All Speed Bay agent modules**: `conventions.py` (commit/PR contract middleware), `linear_guard.py` (OPE-23 self-trigger guard), `docker_sandbox.py` (OPE-7 docker backend), `forge_rules/` (OPE-14 atomicity-cap + commit-hygiene rules as pure functions; not yet wired into the OPE-8 gate middleware) |
