...
| 7 | `speedbay_linear_guard.dispatch_comment_once(...)` background delivery claim/commit (OPE-56) | inside `linear_webhook()` in `agent/webhooks/linear_routes.py`, immediately before background-task dispatch after validation and repo resolution; logic lives in `agent/speedbay/linear_guard.py` |
...
| 14 | `retention_router` import + `include_router` (OPE-177) | `create_app()` in `agent/api/app.py`; the loopback-only conditional-delete endpoint lives in `agent/speedbay/retention_api.py` |
| 15 | `model_settings_router` host-only API registration (OPE-134) | import + `include_router` in `agent/api/app.py`; commit operation lives in `agent/speedbay/model_settings.py` |
...
| `agent/server.py` | Imports + four entries in the `get_agent()` middleware list (`SpeedbayConventionsMiddleware`, `PlanModePolicyMiddleware` (OPE-183), `PRStandardsMiddleware`, `QualityGatesMiddleware`), plus the wrapped `request_pr_review` tool import (OPE-100), plus `pull.rebase true` in `_configure_git_identity` (OPE-109), plus the `ModelCallTimeoutMiddleware` ordering before fallback (OPE-125), plus notification-failure containment in `PrepareAgentRunMiddleware._prepare` (OPE-132) | Sanctioned registration point; no alternative seam. The plan-mode policy strips the model-initiated tool and guidance from Linear-triggered runs (source-aware; Slack/dashboard planning is untouched) while leaving upstream's hot prompt file unpatched. The rebase default rides the only sandbox-global git-config call, so branch syncs never create merge commits that linear-history rulesets reject. The timeout must wrap fallback to enforce one total model-call/fallback wall-clock budget. The OPE-132 guard belongs at this exception-preservation boundary: no sanctioned seam exists, and moving it into the shared helper would alter already-contained callers; re-check both run-setup guards after upstream merges. |
...
| `agent/webhooks/linear.py` | Best-effort trace-comment failure containment after run dispatch (OPE-158), plus the `ACCEPTANCE_CRITERIA_CONTRACT` prompt constant (OPE-182) | The run dispatch is the durable delivery commit point; trailing observability failure must not release the comment claim and launch a duplicate run. |
| `agent/utils/model.py` | Imports + one marked block at the top of `make_model()`: current-toggle check before the cache-checked `subscription_model(...)` short-circuit (OPE-60, OPE-145) | The single chokepoint every graph's model construction routes through; checking the toggle before the subscription cache makes later constructions follow runtime toggle changes without altering models already returned. The block must precede the `openai:` base_url default it overrides. |
...
| `agent/middleware/model_call_timeout.py` | Detached-task deadline wait in `ModelCallTimeoutMiddleware.awrap_model_call` (OPE-125) | Shared middleware is used by the main agent, reviewer, chat, and three subagent construction paths; replacing it through org-owned indirection would require registrations in all six paths. The direct edit stops waiting at deadline when cancellation is suppressed while retaining and observing the detached task. |
...
| `agent/dashboard/team_settings.py` | `upsert_team_settings` reads, merges only caller-set fields, and writes under the shared OPE-134 lock | This dashboard writer replaces the same shared record as the org-owned read/merge/write model commit; serializing and merging both prevents process-local lost updates and partial-payload clobbering. |
| `agent/utils/ttl_cache.py` | Per-key `_GENERATIONS` plus global `_EPOCH` guard in `cached`/`_refresh_stale_entry`; `invalidate()`/`clear()` bump them so in-flight loader results are discarded (OPE-134) | The generation check must run inside the shared cache's write path — the OPE-134 commit's warmed-cache verification depends on invalidation being immune to racing loader repopulation, and an org-owned wrapper cannot intercept writes made by loaders already in flight. |
| `ui/src/features/agents/utils/diffUtils.ts` | `readStoredDiffOverflow`, `writeStoredDiffOverflow`, and `handleDiffOverflowStorage` handle denied storage and `StorageEvent.key === null` (OPE-135) | A new adapter would split this small shared store across files and still require an upstream registration. Re-run `cd ui && pnpm test --run DiffWrapToggle diffUtils` after each upstream merge; remove this `SPEEDBAY DEVIATION (OPE-135)` if upstream absorbs equivalent `SecurityError` fallback and clear-event behavior. |
